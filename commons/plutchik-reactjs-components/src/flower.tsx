    ...
    render(): React.ReactNode {
        ...
        <svg xmlns='http://www.w3.org/2000/svg' xmlnsXlink='http://www.w3.org/1999/xlink' width={this.props.width} height={this.props.width} viewBox={`-${w/2} -${w/2} ${w} ${w}`}>
            {Object.values(Emotion).map((v, i)=> {
                ...
                const XC1 = Math.round(w/2*.7 * Math.sin(ac1));
                const XC2 = Math.round(w/2*.7 * Math.sin(ac2));
                const YC1 = -Math.round(w/2*.7 * Math.cos(ac1));
                const YC2 = -Math.round(w/2*.7 * Math.cos(ac2));
                return <g key={i}>
                    <path style={{stroke: 'silver', fill: "transparent", strokeWidth: "1px", strokeDasharray:"1 1"}} d={`M 0,0 L ${XC1},${YC1} Q ${XV},${YV} ${XC2},${YC2} L 0,0, z`}/>
                    <path style={{fill: `var(--plutchik-${v}-color)`}} d={`M 0,0 L ${xc1},${yc1} Q ${xv},${yv} ${xc2},${yc2} L 0,0, z`}/>
                </g>
            })}
        </svg>
        </span>;
